<x-app-layout>
        ...
        <div class="flex flex-col lg:flex-row justify-between items-start lg:items-center mb-6 gap-4">
            <div>
                ...
                <div class="text-xs text-gray-400 mt-1">
                    <i class="fas fa-sync-alt me-1"></i>Última sincronización: {{ \Carbon\Carbon::parse($ultimaSincronizacion)->diffForHumans() }}
                </div>
                @endif
            </div>
            ...
            <div class="flex flex-wrap items-center gap-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <select name="cola" class="border-gray-300 rounded-md shadow-sm text-sm focus:ring-indigo-500 focus:border-indigo-500">
                        ...
                        @foreach($colasDisponibles as $cola)
                            <option value="{{ $cola }}" {{ ($colaFiltro ?? '') == $cola ? 'selected' : '' }}>Cola {{ $cola }}</option>
                        @endforeach
                    </select>
                </div>
                @endif
            ...
            <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-blue-500">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <h2 class="text-2xl font-bold text-gray-800">{{ number_format($totales['volumen']) }}</h2>
            ...
            <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-green-500">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <h2 class="text-2xl font-bold text-green-600">{{ number_format($totales['atendidas']) }}</h2>
                        <p class="text-xs text-gray-400">{{ $totales['volumen'] > 0 ? round(($totales['atendidas'] / $totales['volumen']) * 100, 1) : 0 }}% del total</p>
                    </div>
                    <div class="text-3xl text-green-500 opacity-25"><i class="fas fa-check-circle"></i></div>
                </div>
            </div>

            <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 {{ ($totales['abandono_valor'] ?? 0) > 20 ? 'border-red-500' : (($totales['abandono_valor'] ?? 0) > 15 ? 'border-yellow-500' : 'border-orange-500') }}">
                <div class="flex justify-between items-center">
                    <div>
                        <h6 class="text-gray-500 uppercase text-xs font-bold">% Abandono</h6>
                        <h2 class="text-2xl font-bold {{ ($totales['abandono_valor'] ?? 0) > 20 ? 'text-red-600' : (($totales['abandono_valor'] ?? 0) > 15 ? 'text-yellow-600' : 'text-orange-600') }}">
                            {{ $totales['abandono_pct'] }}
                        </h2>
                        <p class="text-xs text-gray-400">{{ number_format($totales['abandonadas']) }} abandonadas</p>
                    </div>
                    <div class="text-3xl {{ ($totales['abandono_valor'] ?? 0) > 20 ? 'text-red-500' : 'text-orange-500' }} opacity-25"><i class="fas fa-phone-slash"></i></div>
                </div>
            </div>
            ...
            <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-cyan-500">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <h2 class="text-2xl font-bold text-cyan-600">{{ $totales['tiempo_espera_promedio'] }}</h2>
            ...
            <div class="bg-white p-4 rounded-lg shadow-sm border-l-4 border-purple-500">
                <div class="flex justify-between items-center">
                    <div>
                        ...
                        <h2 class="text-2xl font-bold text-purple-600">{{ count($totales['agentes']) }}</h2>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @forelse($kpisPorHora as $hora => $datos)
                            @if($datos['volumen'] > 0)
                            <tr class="hover:bg-gray-50 transition-colors">
                                <td class="px-6 py-4 whitespace-nowrap">
                                    <span class="font-medium text-gray-900">{{ $datos['hora'] }}</span>
                                </td>
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    ...
                                    @endphp
                                    <span class="font-medium {{ $colorEspera }}">{{ $datos['tiempo_espera_promedio'] }}</span>
                                </td>
                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                    <span class="font-medium text-gray-900">{{ $datos['asa'] }}</span>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="flex flex-wrap gap-1">
                                        @forelse($datos['agentes'] as $agente)
                    ...
                    <tfoot class="bg-gray-100">
                        <tr class="font-semibold">
                            ...
                            <td class="px-6 py-4 text-center text-gray-900">{{ $totales['volumen'] }}</td>
                            <td class="px-6 py-4 text-center text-green-700">{{ $totales['atendidas'] }}</td>
                            <td class="px-6 py-4 text-center {{ ($totales['abandono_valor'] ?? 0) > 20 ? 'text-red-700' : 'text-orange-700' }}">{{ $totales['abandono_pct'] }}</td>
                            <td class="px-6 py-4 text-center text-gray-900">{{ $totales['tiempo_espera_promedio'] }}</td>
                            <td class="px-6 py-4 text-center text-gray-900">-</td>
                            <td class="px-6 py-4 text-purple-700">{{ count($totales['agentes']) }} agentes</td>
                        </tr>
                    </tfoot>
                    @endif
                </table>
        ...
        <div class="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
            ...
            <div class="p-4 space-y-2">
                @foreach($alertas as $alerta)
                    <div class="flex items-center gap-3 p-3 rounded-lg {{ $alerta['tipo'] === 'danger' ? 'bg-red-50 text-red-700' : 'bg-yellow-50 text-yellow-700' }}">
                        <i class="fas fa-{{ $alerta['icono'] }} text-lg"></i>
                        <span>{{ $alerta['mensaje'] }}</span>
                    </div>
                @endforeach
            </div>
        </div>
        @elseif($totales['volumen'] > 0)
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium bg-teal-100 text-teal-800">
                                    <i class="fas fa-phone-volume me-2"></i>{{ $datos['cola'] }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="font-medium text-gray-900">{{ $datos['volumen'] }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                    {{ $datos['atendidas'] }}
                            ...
                            <td class="px-6 py-4 text-center">
                                ...
                                @if($cantAgentes > 0)
                                <button type="button" 
                                        onclick="mostrarAgentes('{{ $datos['cola'] }}')"
                                    ...
                                    <i class="fas fa-users me-1"></i>{{ $cantAgentes }} agente{{ $cantAgentes > 1 ? 's' : '' }}
                                </button>
                                @else
        ...
        <div class="bg-white rounded-lg shadow-sm overflow-hidden mb-6">
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($rendimientoAgentes as $agente => $datos)
                        <tr class="hover:bg-purple-50 transition-colors">
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-medium text-gray-900">Ext. {{ $datos['agente'] }}</div>
                                        <div class="text-xs text-gray-500">{{ $datos['llamadas_totales'] }} llamadas asignadas</div>
                                    </div>
                                </div>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-bold bg-green-100 text-green-800">
                                    {{ $datos['llamadas_atendidas'] }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                @php
                                ...
                                @endphp
                                <span class="font-medium {{ $tasaColor }}">{{ $datos['tasa_atencion'] }}%</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="font-medium text-gray-900">{{ $datos['tiempo_total_formato'] }}</span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                    {{ $datos['tiempo_promedio_formato'] }}
                                </span>
                            </td>
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-cyan-100 text-cyan-800">
                                    {{ $datos['tiempo_espera_promedio_formato'] }}
                                </span>
                            </td>
                        </tr>
                        @endforeach
    ...
    <script>
        ...
        const kpisPorHora = @json(array_values($kpisPorHora));
        const kpisPorCola = @json(array_values($kpisPorCola ?? []));
        ...
        const agentesPorCola = @json($agentesPorCola ?? []);
        ...
        async function ejecutarSincronizacion() {
            ...
            try {
                const response = await fetch('{{ route("stats.sync-colas") }}', {
                    method: 'POST',
                    headers: {
